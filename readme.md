...
- [x] Change the way settings are stored and read
  - [x] a) Add an Auto-Generated basic settings file when first run
  - [x] b) Read from settings file on start
- [x] Add Sabermod toolchain download, compile and install options
  - [x] a) Android Rom Toolchains
  - [x] b) Android Kernel Toolchains
- [x] Create a user selectable list for downloading different android ROM source code
  - [x] a) DirtyUnicorns Manifest Repo
  - [x] b) OMNIrom Manifest Repo
  - [x] c) CyanogenMOD Manifest Repo
  - [x] d) Pacman Manifest Repo
  - [x] e) Add ability to add your own repo
- [ ] Add checks for installed SaberMod Toolchains
  - [x] Add checks on startup
  - [ ] Add check before Rom compile,then ask which toolchain to use for Rom build
  - [ ] Add check before Kernel compile,then ask which toolchain to use for Kernel build
...
- [ ] Change the way RomBuilder & KernelBuilder cores work
  ...
  - [ ] c) Change hardcoded directories to a more robust dynamic way to handle additional user folders
- [ ] Create a user selectable list for choosing which android device to build for
