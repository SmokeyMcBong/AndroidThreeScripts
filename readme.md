...
- [ ] Change the way settings are stored and read
  - [x] a) Add an AutoGenerated basic settings file when first run
  - [x] b) Read from settings file on start
  - [ ] c) Merge other settings from user.prefs into settings file
- [ ] Add Sabermod toolchain download, compile and install options
  - [ ] a) Android Rom
  - [ ] b) Android Kernel
...
- [ ] Create a user selectable list for downloading different android ROM source code
  - [ ] a) DirtyUnicorns Manifest Repo
  - [ ] b) OMNIrom Manifest Repo
  - [ ] c) CyanogenMOD Manifest Repo
  - [ ] d) Pacman Manifest Repo
